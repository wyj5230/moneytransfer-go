...
func httpRequest(method string, url string, requestBody io.Reader, username string, secret string) []byte {
	...
	if requestErr != nil {
		log.Printf("invalid request %s", requestErr)
	}
	req.SetBasicAuth(username, secret)
	req.Header.Add("Content-Type", "application/json")
	resp, responseErr := client.Do(req)
	if responseErr != nil {
		log.Printf("invalid response %s", responseErr)
	}
	body, responseBodyErr := ioutil.ReadAll(resp.Body)
	if responseBodyErr != nil {
		log.Printf("invalid responsebody %s", responseBodyErr)
	}
	if resp.StatusCode != 200 && resp.StatusCode != 201 {
		log.Printf("Bad request: %s", string(body))
	}
	return body
}
...
func attachmentUploadRequest(url string, fileType string, fileName string, filePath string,
	...
	if fileErr != nil {
		log.Printf("invalid file %s", fileErr)
	}
	defer file.Close()
	...
	if formErr != nil {
		log.Printf("invalid form data %s", formErr)
	}
